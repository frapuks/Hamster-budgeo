 ...
 * Le rôle `provisions` a son propre écran (VueProvisions) et n'apparaît pas ici.
 */
function heros(compte: CompteCalcule): { libelle: string; montantCents: number } {
  return compte.role === 'prelevements'
    ? { libelle: 'Reste à sortir', montantCents: compte.resteASortirCents }
    : { libelle: 'Il doit encore couvrir', montantCents: compte.besoinDuCycleCents }
}
  ...
  if (compte.role === 'provisions') {
    return (
      <Stack spacing={3} sx={{ pb: 2 }}>
        <EnTeteCompte compte={compte} onRetour={() => navigate('/')} />
